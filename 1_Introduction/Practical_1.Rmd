...
c() in R is used to create a vector by explicitely giving values of a common type
...
This creates a generic vector that contains 1,2,3, Hi. R recognizes this not as a numeric data, but as a character data because it is heterogenous.


* Would the following be valid?

`c(1, 2, 3, "Hi") + 1 `
No. Becasue we are trying to add a number to a non-numerical value.
* How about 

`c(1, 2, 3, FALSE) + 1 `
Yes. FALSE in R is a logical argument, and assigns the value 0 (against TRUE which is 1). Hence, this can be valid, though not very good housekeeping of data, in my opinion.
...
library(tidyverse)
?qplot
qplot(x_axis, y_axis, geom = "path", main = "Gas Consumption by car", xlab = "Gas (litres)", ylab = "Distance (m)")
