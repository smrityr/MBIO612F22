...
The geometric points are orange in colour instead of blue.the aes() function gathered to highlight the aesthetics for x and y. We need to separate the colour from that command, hence needed the bracket.
...
```{r}
# Write your code here
mpg$class_numeric <- as.integer(mpg$class)
...
```{r}
# Write your code here
data_cty_15 <- subset(mpg, cty == 15, select = c(manufacturer, model))
print(data_cty_15)
```
...
sizeMass <- cbind.data.frame(x, y)
sizeMass <- as_tibble(sizeMass)
names(sizeMass)[names(sizeMass) == "x"] <- "size"
names(sizeMass)[names(sizeMass)== "y"] <- "mass"
